...
process.stdout.write(`(vulkan-api) Using Vulkan v${vkVersion}\n`);
...
if (!fs.existsSync(`${generatedPath}/interfaces.js`)) {
  process.stderr.write(`(vulkan-api) Failed to load interfaces for v${vkVersion} from ${generatedPath}\n`);
  process.stderr.write(`(vulkan-api) Your platform might not be supported\n`);
  throw `Exiting..`;
}
...
if (disableValidationChecks) {
  process.stdout.write(`(vulkan-api) Validation checks are disabled\n`);
  module.exports = require(`${generatedPath}/interfaces-no-validation.js`);
} else {
  process.stdout.write(`(vulkan-api) Validation checks are enabled\n`);
