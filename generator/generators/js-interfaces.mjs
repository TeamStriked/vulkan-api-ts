...
function getSetterProcessor(member) {
  ...
  switch (type) {
    ...
    if (value !== null ${ validate ? `&& value.constructor === Function` : `` }) {
      ...
      this._${member.name} = value;
      this._${member.name}CallbackProxy = new vulkanApi.$CallbackProxy(MITM, module.exports);
      this.memoryView.set${instr}(${offset}, vulkanApi.$vulkanCallbackFunctionPointers["${functionPointerAccessor}"], ${endianess});
      ${pUserData}
    } else if (value === null) {
      this._${member.name} = null;
      this.memoryView.set${instr}(${offset}, BI0, ${endianess});
...
export default function(astReference, includeValidations, disableMinification, calls, handles, structs) {
  ...
  output += `  VK_VERSION_PATCH: vulkanApi.VK_VERSION_PATCH,\n`;
  output += `  VK_API_VERSION_1_0: vulkanApi.VK_API_VERSION_1_0,\n`;
  output += `  VK_API_VERSION_1_1: vulkanApi.VK_API_VERSION_1_1,\n`;
  output += `  VK_API_VERSION_1_2: vulkanApi.VK_API_VERSION_1_2,\n`;
  output += `  vkUseDevice: vulkanApi.vkUseDevice,\n`;
  output += `  vkUseInstance: vulkanApi.vkUseInstance,\n`;
  // add VulkanWindow
  output += `  VulkanWindow: vulkanApi.VulkanWindow,\n`;
  calls.map(call => {
    output += `  ${call.name}: vulkanApi.${call.name},\n`;
  });
  handles.map(handle => {
    output += `  ${handle.name},\n`;
  });
