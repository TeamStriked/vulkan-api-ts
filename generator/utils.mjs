...
export function funcByType(val) {
  if (val.toString() == "uint32_t") {
    return `createUint32ToArrayOfV8Numbers`;
  }
  else if (val.toString() == "int32_t'") {
    return `createInt32ToArrayOfV8Numbers`;
  }
  else if (val.toString() == "float") {
    return `createFloatToArrayOfV8Numbers`;
  }
  else if (val.toString() == "VkFragmentShadingRateCombinerOpKHR") {
    return `createFromShadingArrayOfV8Numbers`;
  }
  else {
    throw new error(`Cannot resolve type func for "${val}"`);
  }
};
